...
import { getJournalVolumes, getIssueSpan } from '../data/journalsArchiveData';
...
export const JournalSubpage: React.FC = () => {
  ...
  if (subpage === 'archives' || params.volumeId) {
    ...
    const selectedIssue = params.issueId || null;

    const volumes = getJournalVolumes(journal.id);

    const selectedVolumeData = selectedVolume
      ? volumes.find((v) => v.volume === selectedVolume)
      : undefined;
    const selectedIssueData =
      selectedIssue && selectedVolumeData
        ? selectedVolumeData.issues.find((i) => i.id === selectedIssue)
        : undefined;

    const allJournalArticles = getArticlesByJournalId(journal.id);
    const issueArticles =
      selectedVolume && selectedIssueData
        ? allJournalArticles.filter((a) => {
            const [issueStart, issueEnd] = getIssueSpan(selectedIssueData.id);
            return a.volume === selectedVolume && a.issue >= issueStart && a.issue <= issueEnd;
          })
        : [];
          ...
          {selectedVolume && selectedIssue ? (
            <div>
              ...
              {issueArticles.length > 0 ? (
                ...
                <div className="p-8 text-center text-gray-500 text-xs bg-gray-50 border border-gray-200">
                  No articles are available in this volume and issue yet.
                </div>
              )}
            </div>
          ) : selectedVolume ? (
            <div>
              ...
              {selectedVolumeData ? (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {selectedVolumeData.issues.map((iss) => (
                    <Link
                      key={iss.id}
                      to={`/journals/${journal.id}/volume/${selectedVolume}/issue/${iss.id}`}
                      className="p-3 border border-gray-300 text-center hover:bg-gray-100 transition-colors text-xs font-bold text-gray-800"
                    >
                      Issue {iss.id}
                      <span className="block mt-1 text-[10px] font-normal text-gray-500">
                        {iss.period}
                      </span>
                    </Link>
                  ))}
                ...
                <div className="p-8 text-center text-gray-500 text-xs bg-gray-50 border border-gray-200">
                  This volume has not been published for this journal yet.
                </div>
              )}
            </div>
          ) : (
            <div>
              ...
              {volumes.length > 0 ? (
                ...
                <div className="p-8 text-center text-gray-500 text-xs bg-gray-50 border border-gray-200">
                  No volumes have been published for this journal yet.
                </div>
              )}
            </div>
          )}
